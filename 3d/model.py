...
def unet(pretrained_weights = None,input_size = (64,64,32,1),l_rate=0.01,loss_func=dice_coef_loss):
    ...
    conv_last = keras.layers.Conv3D(2, 3, padding="same", activation="relu", kernel_initializer = 'he_normal')(up4)
    outputs = keras.layers.Conv3D(1, 1, padding="same", activation="sigmoid")(conv_last)
...
def unet_norm(pretrained_weights = None,input_size = (64,64,32,1),l_rate=0.01,loss_func=dice_coef_loss):
    inputs = keras.layers.Input(input_size)
