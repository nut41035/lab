...
def predict(MODEL, base_path, img_name, save_mode=0):
    """ save_mode : 0 = not save
                    1 = save all
                    2 = save only contain color area
    """
    ...
    for i in range(32):
        ...
        save = True if save_mod==1 else False
        for j in range(64):
            for k in range(64):
                m_pixel = mask[j,k,i]
    ...
    return TP,FP,FN,prediction

def predict_folder(MODEL, base_path, save_mode=True):
    ...
    for path in tqdm(files, desc='images in folder', leave=False):
        ...
        TP, FN, FP = predict(MODEL, base_path, img_name, save_mode)
        TP_sum += TP
        FN_sum += FN
        FP_sum += FP
